...
#iloveweb #web #platform`)}}function I(e){try{return new URL(location.href).searchParams.get(e)}catch(t){console.log(t)}}function T(e,t={}){dataLayer.push({event:e,...t})}var j={html:D,css:N,javascript:E},W=10,S={mode:"QUIZ",questions:[],load({lang:e,tasks:t}){t.forEach(o=>{this.questions.push({lang:e,question:o})})},init(){let e=I("q");if(e){let t=this.questions.find(({question:o})=>o.id===e);t?(this.mode="SINGLE",this.questions=[t]):console.error(`Question with id "${e}" not found.`)}if(this.mode==="QUIZ"){let t=this.questions.length,o;for(;t!=0;)o=Math.floor(Math.random()*t),t--,[this.questions[t],this.questions[o]]=[this.questions[o],this.questions[t]];this.questions=this.questions.slice(0,W),console.log(`Questions in total: ${this.questions.length}`)}},check(e,t,o,s){let{lang:r,question:a}=this.questions[t],i=j[r]?j[r]:E;try{a.validator(i(e))&&o()}catch(u){s(O(u.toString().replace(/</gi,"&lt;")))}}};function D(e){return html2json(e)}function N(e){var t=new cssparser.Parser;return t.parse(e)}function E(e){return e}function O(e){return e.length>500?`${e.substr(0,500)}...`:e}window.ILoveWeb=S;var n=S;var Q=["/js/cssparser.min.js","/js/html2json.js","/js/htmlparser.min.js","/js/lodash.min.js","/js/parsel.js","/js/specificity.js","https://cdnjs.cloudflare.com/ajax/libs/lottie-web/5.9.6/lottie.min.js","https://cdnjs.cloudflare.com/ajax/libs/gsap/3.11.3/gsap.min.js","/imgs/Cycle_custom_icon.json","/questions/css.js","/questions/html.js","/questions/javascript.js"];async function F(e,t){let o=0;m({content:`
    ...
    `}),b(".loader"),await Promise.all(e.map(async s=>{await $(s),o+=1,c(".value").style.width=`width:${Math.ceil(o/e.length*100)}%;`})),gsap.to(".loader",{y:"-100px",opacity:0,ease:"back.in(1.7)",onComplete:()=>{typeof _<"u"&&(window.get=_.get),t()}})}function G(){m({content:`
    ...
    `,onRender(){let e=0,t,o=0,s,r=c(".editor"),a=c(".editor .question"),i=c(".editor textarea"),u=c(".editor .console"),d=c(".timer");function w(){let l=n.questions[e];m({content:`<div class="z2">${l.lang.toUpperCase()}: ${l.question.text}</div>`,container:a}),gsap.fromTo(a,{y:"100px",opacity:0},{y:0,opacity:1})}function f(){t&&t.stop(),clearInterval(s),r.style.display="block",d&&(d.innerHTML=`${n.questions.length}/${n.questions.length} \xB7 ${h(o)}`);let l=n.questions.length,p=`
          <br /><br />
